...
const DEFAULT_TEMPLATE_QUESTIONS = [
  ...
  },
  {
    ...
    label: "Cuántos años lleva viviendo en ese terreno",
    input_type: "number",
    is_required: true,
    section_key: "situacion_terreno",
    section_title: "Situacion del Terreno",
...
export async function loadFormDefinition(supabase, rawSlug) {
  const form = await getOrCreateFormBySlug(supabase, rawSlug);
...
export async function seedTemplateQuestionsIfEmpty(supabase, formId) {
  ...
  if ((count || 0) > 0) {
    // Backward-compatible typo fix for already-seeded environments.
    await supabase
      .from("assessment_questions")
      .update({
        label: "Cuántos años lleva viviendo en ese terreno",
      ...
      .eq("form_id", formId)
      .eq("question_key", "anios_en_terreno")
      .eq("label", "Cuantos anos lleva viviendo en ese terreno");
